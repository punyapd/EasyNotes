...
const ResponsiveSidebar = () => {
  ...
  const tokenString = sessionStorage.getItem("user_info");
        ...
        <ul className="pt-6">
          ...
          {Menus.map((menu, index) => (
            <Link href={menu.path} key={index}>
              <li
                className={`flex  active items-center gap-x-6 p-3 text-base font-normal rounded-lg cursor-pointer dark:text-white hover:bg-gray-200 w-full dark:hover:bg-green-700 hover:bg-theme
                        ${menu.gap ? "mt-9" : "mt-2"}  ${
                  pathname == menu.path ? "bg-theme" : ""
                }
                }`}
              >
